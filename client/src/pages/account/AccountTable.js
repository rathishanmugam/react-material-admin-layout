...
// console.log('process.env.js', process.env);
...
const AccountTable = ({
    ...
    const deleteItem = _id => {
        ...
        if (confirmDelete) {
            // fetch('http://localhost:8081/api/account/' + _id, {
                fetch(`${api}/api/accout/${_id}`, {
                method: 'delete',
                headers: {
        ...
        <Paper className={classes.root}>
            {/*<div className={classes.tableWrapper}>*/}
                ...
                <Table>
                    <TableHead>
                        <TableRow>
                            <StyledTableCell align="center" onClick={() => handleSort('accountNo')}>Account No
                                <span>{col.colToSort === 'accountNo' ?
                                    (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                            </StyledTableCell>
                            <StyledTableCell align="center" onClick={() => handleSort('particulars')}>particulars
                                <span>{col.colToSort === 'particulars' ?
                                    (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                            </StyledTableCell>
                            <StyledTableCell align="center" onClick={() => handleSort('credit')}>credit
                                <span>{col.colToSort === 'credit' ?
                                    (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                            </StyledTableCell>
                            <StyledTableCell align="center" onClick={() => handleSort('debit')}>debit
                                <span>{col.colToSort === 'debit' ?
                                    (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                            </StyledTableCell>
                            <StyledTableCell align="center" onClick={() => handleSort('createdOn')}>createdOn
                                <span>{col.colToSort === 'createdOn' ?
                                    (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                            </StyledTableCell>
                    ...
                    <TableBody>
                        {
                            ...
                            items.map(row => (
                                ...
                                <StyledTableRow key={row._id}>
                                    ...
                                    <StyledTableCell align="center">{row.createdOn.slice(0,15)}</StyledTableCell>
                                    <StyledTableCell align="center">
                                        <AccountModel buttonLabel="Edit" item={row} updateState={updateState}/>{' '}
