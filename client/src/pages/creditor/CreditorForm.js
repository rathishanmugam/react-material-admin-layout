...
class CreditorForm extends React.Component {
    ...
    submitFormAdd = e => {
        ...
        const array = [];
        axios.post(`http://localhost:8081/api/creditor`, obj)
            .then(res => {
                console.log(res);
    ...
    submitFormEdit = e => {
        ...
        const obj = {...this.productForm.value};
        const array = [];
        axios.put(`http://localhost:8081/api/creditor/${this.addressId}/${this.props.item._id}`, obj)
            .then(res => {
                console.log(res);
                console.log(res.data);
                console.log('the  sales saved item', res.data);
    ...
    render() {
        if (this.props.item) {
            ...
            this.productForm.get('gstIM').patchValue(this.props.item.gstIM);
            ...
            <>
                <FieldGroup
                    ...
                    render={({value, pristine, invalid, hasError, handler, touched}) => (
                      ...
                      <span style={styles.error}>
                        {(touched &&
                            (hasError("required") && "gstIM is required"))}
                      </span>
                                            </div>
                                        </div>
                                    )}
