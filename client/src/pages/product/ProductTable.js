...
const ProductTable = ({
    ...
    const deleteItem = _id => {
        ...
        if (confirmDelete) {
            fetch('http://localhost:8081/api/product/' + _id, {
                method: 'delete',
                headers: {
            ...
            <Table>
                <TableHead>
                    <TableRow>
                        ...
                        <StyledTableCell align="center" onClick={() => handleSort('modelNo')}>modelNo
                            <span>{col.colToSort === 'modelNo' ?
                                (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                        </StyledTableCell>
                        <StyledTableCell align="center" onClick={() => handleSort('HSNCodeNo')}>HSNCodeNo
                            <span>{col.colToSort === 'HSNCodeNo' ?
                                (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                        </StyledTableCell>
                        <StyledTableCell align="center" onClick={() => handleSort('companyName')}>companyName
                            <span>{col.colToSort === 'companyName' ?
                                (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                        </StyledTableCell>
                        <StyledTableCell align="center" onClick={() => handleSort('productType')}>productType
                            <span>{col.colToSort === 'productType' ?
                                (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                        </StyledTableCell>
                        <StyledTableCell align="center" onClick={() => handleSort('qty')}>Stock
                            <span>{col.colToSort === 'qty' ?
                                (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                        </StyledTableCell>
                        <StyledTableCell align="center" onClick={() => handleSort('rate')}>rate
                        <span>{col.colToSort === 'rate' ?
                            (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                    </StyledTableCell>
                        <StyledTableCell align="center" onClick={() => handleSort('gstRate')}>gstRate
                            <span>{col.colToSort === 'gstRate' ?
                                (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                        </StyledTableCell>
                        <StyledTableCell align="center" onClick={() => handleSort('sgstRate')}>sgstRate
                            <span>{col.colToSort === 'sgstRate' ?
                                (col.reverse ? <ArrowUpwardIcon/> : <ArrowDownwardIcon/>) : null}</span>
                        </StyledTableCell>
                ...
                <TableBody>
                    {
                        ...
                        items.map(row => (
                            ...
                            <StyledTableRow key={row._id}>
                                ...
                                <StyledTableCell align="center">{row.HSNCodeNo}</StyledTableCell>
                                ...
                                <StyledTableCell align="center">{row.gstRate} </StyledTableCell>
                                <StyledTableCell align="center">{row.sgstRate} </StyledTableCell>

                                <StyledTableCell align="center">
                                    <div style={{width: "130px"}}>
                                        <ProductModel buttonLabel="Edit" item={row} updateState={updateState}/>{' '}
